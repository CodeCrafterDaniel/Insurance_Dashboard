...
if page == "Обзор страхового рынка":
    ...
    with col2:
        ...
        st.info(
            ...
            * За анализируемый период **квартальные сборы рынка выросли почти в 3 раза** (с 400 до 1300 млрд руб.). **Исторический максимум** сборов зафиксирован в IV кв. 2024 года.
        ...
        with col2:
            ...
            st.info(
                ...
                **Инсайт (Количество выплат)**
                ...
                Наибольшая доля убытков  **в {year} году**
            ...
            st.info(
                ...
                **({claim_frequency_share:.1f}% от общего количества выплат)**,
                на данный вид приходится **{claim_sum_share:.1f}% всех денежных выплат на рынке**.

                Это указывает на высокую среднюю стоимость одного убытка в данном направлении.
                """,
                icon="💡"
            )
    ...
    with left:
        ...
        stages = [
            "Выбрали страхование путешественников",
            "Заполнили данные для расчета",
            "Получили расчет",
        ...
        st.subheader("Допущения")

        st.info(
            """
            Для оценки количества оформленных полисов в 2025 году предполагается сохранение доли СберСтрахования на рынке туристического страхования на уровне 2021 года.

            Источник: https://sberbankins.ru/about/news/sberstrakhovanie-v-2021-godu-vyplatila-puteshestvennikam-bolee-400-mln-rubley/
            """
        )
    ...
    with right:
        ...
        st.info(
            """
            **Большой отток на этапе «Расчет → Оформление»**

            Почти **70% пользователей** видят цену и не переходят к оформлению - это критическая потеря. Из 4.3M тех, кто получил расчет, только 1.3M переходят к покупке.

            **Почему это происходит:**

            * Пользователи сравнили расчет с рыночными ценами и не нашли выгодное предложение
            * После расчета появляются дополнительные оплачиваемые опции, что отпугивает
            * Пользователь не разбирается в дополнительных опциях и уходит

            **Идеи как исправить:**

            * Показать ориентировочную цену **до** заполнения данных
            * Добавить сравнение с конкурентами
            * Ретаргетинг на ушедших после расчета с предоставлением скидки
            """
        )
    ...
    with left:
        buyers_df = pd.DataFrame({
            ...
            "Buyers": [670, 718, 750, 1010, 1100, 1170]
        })
        ...
        st.info(
            """
                **Инсайт**
                ...
                В **2022 году** зафиксирован аномальный скачок спроса на страхование путешественников. 
                Объем продаж вырос на 40%, что существенно превышает прирост прошлых лет (9–15%). 
                В последующие периоды сформировавшийся объем рынка зафиксировался и падения спроса не произошло, 
                однако темпы прироста стабилизировались и вернулись к историческому тренду в 9–15%.
                """,
            icon="💡"
        )
    ...
    with right:
        revenue_df = pd.DataFrame({
            "Years": [2020, 2021, 2022, 2023, 2024, 2025],
            "Revenue": [900, 1020, 1140, 1900, 2300, 2778]
        })
        ...
        st.info(
            """
                **Инсайт**
                ...
                В 2023 году зафиксирован качественный сдвиг в динамике рынка. 
                Темпы роста совокупных страховых премий существенно ускорились, превысив средние показатели 
                периода 2020–2022 годов.
                """,
            icon="💡"
        )
    ...
    fig_comp.update_layout(
        title=
        'Количество покупателей "Страховки для путешествий"',
        height=420,
        showlegend=False,
    ...
    st.info(
        ...
        **Инсайт**
        ...
        СберСтрахование является уверенным лидером по продаже страховок для путешествий на российском рынке с долей ~25%
        """,
        icon="💡"
    )